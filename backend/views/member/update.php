...
$this->title = Yii::t('main', 'Update Member');
...
{
    $lang_model = \common\models\MembersLang::find()->andFilterWhere(['parent' => $model->id, 'lang' => $language->id])->one();

    if (empty($lang_model) || $lang_model == NULL) {

        $lang_model = new \common\models\MembersLang();
        $action = "/services-lang/create";

    } else {

        $lang_model = \common\models\MembersLang::findOne($lang_model->id);
        $action = "/Members-lang/update/" . $lang_model->id;
    ...
    $langs[] = [
        'id' => $language->id,
        'label' => $language->name,
        'content' => $this->render('/Members-lang/_form', [
            'id' => $language->id,
            'model' => $lang_model,
            'action' => $action,
        ]),
